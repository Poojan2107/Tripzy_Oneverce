...
export default function DestinationError({ error, reset }: { error: Error; reset: () => void }) {
  useEffect(() => { console.error(error); }, [error]);

  return (
    <div className="min-h-[100dvh] bg-background flex flex-col items-center justify-center p-6 text-center">
      <Compass className="w-12 h-12 text-gold mb-4" />
      <h1 className="font-display text-2xl text-night font-light lowercase mb-2">something went wrong</h1>
      <p className="text-sm text-muted/60 font-light max-w-xs mb-6">We couldn&apos;t load this destination. It might be a temporary issue.</p>
