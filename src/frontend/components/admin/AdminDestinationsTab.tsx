                ...
                <tr key={tour.id} className="hover:bg-cream/30 transition-colors">
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-3">
                      <img src={tour.bannerImage} alt={tour.title} loading="lazy" decoding="async" className="w-12 h-12 rounded-xl object-cover border border-warm-gray shrink-0" />
                      <div>
                  ...
                  <td className="py-4 px-6 text-right font-semibold text-ink font-mono text-[11px]">{formatINR(tour.price)}</td>
                  <td className="py-4 px-6 text-right space-x-2 whitespace-nowrap">
                    <button onClick={() => onEdit(tour)} className="w-11 h-11 rounded-lg bg-cream/30 text-stone hover:bg-gold hover:text-white active:scale-95 transition-all duration-200 cursor-pointer inline-flex items-center justify-center border border-warm-gray/60 touch-action-manipulation select-none" title="Edit">
        ...
        {filtered.map((tour) => (
          <div key={tour.id} className="bg-white border border-warm-gray rounded-2xl p-4 shadow-card">
            <div className="flex items-start gap-3">
              <img src={tour.bannerImage} alt={tour.title} loading="lazy" decoding="async" className="w-16 h-16 rounded-xl object-cover border border-warm-gray shrink-0" />
            ...
            <div className="flex gap-2 mt-3 pt-3 border-t border-warm-gray/60">
              ...
              <div className="ml-auto flex gap-2">
                <button onClick={() => onEdit(tour)} className="w-11 h-11 rounded-lg bg-cream/30 text-stone hover:bg-gold hover:text-white active:scale-95 transition-all duration-200 cursor-pointer inline-flex items-center justify-center border border-warm-gray/60" title="Edit">
