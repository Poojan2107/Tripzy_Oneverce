...
export default function AdminView({ tours, wishlistCount, onAddTour, onUpdateTour, onDeleteTour }: AdminViewProps) {
  ...
  const loadData = useCallback(async () => {
    try { setLoadingMetrics(true); const r = await getDashboardMetrics(); if (r.success) setMetrics(r.data); } catch {} finally { setLoadingMetrics(false); }
    try { setLoadingExperiences(true); const r = await getAllExperiences(); if (r.success && r.data) setExperiences(r.data as any); } catch {} finally { setLoadingExperiences(false); }
    try { setLoadingUsers(true); const r = await getAdminUsers(); if (r.success) setUsers(r.data); } catch {} finally { setLoadingUsers(false); }
  }, []);
