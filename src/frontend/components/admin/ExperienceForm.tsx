...
export default function ExperienceForm({ state, actions, onSubmit }: ExperienceFormProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/30 backdrop-blur-sm">
      ...
      <div className="relative w-full sm:max-w-xl bg-warm-white border-l border-warm-gray shadow-elevated h-full flex flex-col p-4 sm:p-6 overflow-y-auto overscroll-behavior-contain">
        ...
        <form onSubmit={onSubmit} className="space-y-4 sm:space-y-5 flex-1">
          ...
          <div className="flex items-center gap-2 pt-2">
            <input type="checkbox" id="expFeatured" checked={state.featured} onChange={(e) => actions.setFeatured(e.target.checked)} className="w-4 h-4 border border-warm-gray rounded bg-white text-gold focus:ring-0 cursor-pointer" />
            <label htmlFor="expFeatured" className="text-xs text-ink font-medium select-none cursor-pointer">Feature this experience on the landing page</label>
          </div>
