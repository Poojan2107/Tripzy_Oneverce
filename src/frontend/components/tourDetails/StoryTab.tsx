      ...
      <motion.div variants={fadeUp}>
        ...
        {tour.moods && tour.moods.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {tour.moods.map((m) => (
              <span key={m} className="px-3.5 py-1 rounded-full bg-secondary-surface text-night border border-border text-micro font-bold uppercase tracking-wider">{m}</span>
            ))}
          </div>
        )}
      </motion.div>
      ...
      {cultural && (
        <motion.div variants={fadeUp} className="p-6 rounded-3xl border border-border bg-white shadow-card space-y-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-micro font-mono uppercase tracking-widest text-muted/60 font-bold">Cultural Context</span>
          </div>
      ...
      {(tour.storyHeadline || tour.storyNarrative) && (
        <motion.div variants={fadeUp} className="space-y-4">
          <div className="flex items-center gap-3">
            {tour.chapterName && <span className="px-2.5 py-1 rounded-full bg-gold/10 text-gold border border-gold/20 text-micro font-bold uppercase tracking-widest">{tour.chapterName}</span>}
            {tour.chapterTitle && <span className="text-micro font-mono text-muted uppercase tracking-widest">{tour.chapterTitle}</span>}
          </div>
      ...
      <motion.div variants={fadeUp} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {tour.localSecret && (
          <div className="p-4 rounded-2xl bg-white border border-border shadow-sm">
            <span className="text-micro font-mono text-coral uppercase tracking-widest font-bold block mb-1.5">Local Secret</span>
            <p className="text-xs text-night/70 font-light leading-relaxed">{tour.localSecret}</p>
          </div>
        )}
        {tour.photographySpot && (
          <div className="p-4 rounded-2xl bg-white border border-border shadow-sm">
            <span className="text-micro font-mono text-gold uppercase tracking-widest font-bold block mb-1.5">Best Photo Spot</span>
            <p className="text-xs text-night/70 font-light leading-relaxed">{tour.photographySpot}</p>
          </div>
        )}
        {tour.signatureExperience && (
          <div className="p-4 rounded-2xl bg-white border border-border shadow-sm">
            <span className="text-micro font-mono text-teal uppercase tracking-widest font-bold block mb-1.5">Signature Experience</span>
            <p className="text-xs text-night/70 font-light leading-relaxed">{tour.signatureExperience}</p>
          </div>
        )}
        {tour.budgetRange && (
          <div className="p-4 rounded-2xl bg-white border border-border shadow-sm">
            <span className="text-micro font-mono text-teal uppercase tracking-widest font-bold block mb-1.5">Budget Guide</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {tour.highlights.map((h, i) => (
              <div key={i} className="flex items-center gap-3 p-3.5 rounded-xl bg-white border border-border">
                <span className="w-5 h-5 rounded-full flex items-center justify-center text-micro font-bold text-white shrink-0" style={{ backgroundColor: accentColor }}>{i + 1}</span>
