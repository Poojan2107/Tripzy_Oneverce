    ...
    <div className="space-y-8">
      <motion.div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4"
        ...
        {[
          ...
          { label: 'Views', icon: Eye, value: totals?.views || 0, color: 'bg-ocean/10 text-ocean' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white border border-border rounded-3xl p-5 sm:p-6 shadow-card">
          <div className="flex items-center justify-between mb-4 pb-3 border-b border-border">
            <h3 className="text-[10px] font-mono font-bold uppercase tracking-[0.25em] text-stone">Top Search Queries</h3>
          ...
          <div className="space-y-2">
            {!metrics?.searchTrends || metrics.searchTrends.length === 0 ? (
              ...
              metrics.searchTrends.map((s: any, idx: number) => (
                <div key={idx} className="flex items-center justify-between text-xs bg-cream/30 px-3 py-2.5 rounded-xl border border-border/60">
                  <span className="font-medium text-night">"{s.query}"</span>
                  <span className="font-bold text-gold bg-gold/10 px-2 py-0.5 rounded-md text-[10px] font-mono">{s.count} hits</span>
                </div>
              ))
            )}
          </div>
        ...
        <div className="bg-white border border-border rounded-3xl p-5 sm:p-6 shadow-card">
          <div className="flex items-center justify-between mb-4 pb-3 border-b border-border">
            <h3 className="text-[10px] font-mono font-bold uppercase tracking-[0.25em] text-stone">Most Viewed Destinations</h3>
            ...
            {!metrics?.destinationPopularity || metrics.destinationPopularity.length === 0 ? (
              ...
              metrics.destinationPopularity.map((d: any, idx: number) => (
                <div key={idx} className="flex items-center justify-between text-xs bg-cream/30 px-3 py-2.5 rounded-xl border border-border/60">
                  ...
                  <span className="font-bold text-ocean bg-ocean/10 px-2 py-0.5 rounded-md text-[10px] font-mono">{d.count} views</span>
                </div>
              ))
            )}
          </div>
        ...
        <div className="bg-white border border-border rounded-3xl p-5 sm:p-6 shadow-card">
          <div className="flex items-center justify-between mb-4 pb-3 border-b border-border">
            <h3 className="text-[10px] font-mono font-bold uppercase tracking-[0.25em] text-stone">AI Recommendation Favorites</h3>
            ...
            {!metrics?.recommendationPopularity || metrics.recommendationPopularity.length === 0 ? (
              ...
              metrics.recommendationPopularity.map((r: any, idx: number) => (
                <div key={idx} className="flex items-center justify-between text-xs bg-cream/30 px-3 py-2.5 rounded-xl border border-border/60">
                  ...
                  <span className="font-bold text-sage bg-sage/10 px-2 py-0.5 rounded-md text-[10px] font-mono">{r.count} times</span>
                </div>
              ))
            )}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 bg-warm-white/70 border border-border p-5 sm:p-6 rounded-3xl">
        <div>
          <h3 className="text-[10px] font-mono font-bold uppercase tracking-[0.25em] text-night mb-4 flex items-center gap-1.5">
        ...
        <div className="space-y-5">
          <div>
            <h3 className="text-[10px] font-mono font-bold uppercase tracking-[0.25em] text-night mb-3 flex items-center gap-1.5">
          ...
          <div>
            <h3 className="text-[10px] font-mono font-bold uppercase tracking-[0.25em] text-stone mb-2.5">Top Requested Travel Styles</h3>
            <div className="flex flex-wrap gap-2">
              {!metrics?.planning?.travelStyles || metrics.planning.travelStyles.length === 0 ? (
                ...
                metrics.planning.travelStyles.map((ts: any, idx: number) => (
                  <span key={idx} className="bg-cream/30 border border-border/60 px-3 py-1 rounded-full text-xs font-medium text-night flex items-center gap-1">
                    {ts.style} <span className="text-[10px] text-stone">({ts.count})</span>
                  </span>
                ))
              )}
            </div>
        ...
        <div className="space-y-4">
          <h3 className="text-[10px] font-mono font-bold uppercase tracking-[0.25em] text-night flex items-center gap-1.5">
            <Sparkles className="w-4 h-4 text-gold" />
            AI Recommendation Feedback
          </h3>
          <div className="space-y-3 pt-2 text-xs">
            <div className="flex justify-between items-center bg-cream/30 p-2.5 rounded-xl border border-border/60">
