                    ...
                    {TOURS_DATA.map((tour, i) => {
                      ...
                      return (
                        <StepOption key={tour.id} selected={isSelected} onClick={() => onDestinationChange(tour.id)} custom={i}
                          className={`group p-2.5 rounded-xl border text-left cursor-pointer flex flex-col justify-between aspect-[4/3] relative overflow-hidden outline-none ${
                            isSelected
                ...
                {selectedTour && (
                  <motion.div className="space-y-3 pt-2" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                    ...
                    <input
                      ...
                      className="w-full h-1 bg-border rounded-lg appearance-none cursor-pointer accent-teal"
                    />
