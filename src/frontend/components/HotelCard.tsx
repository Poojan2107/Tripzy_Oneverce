...
export default function HotelCard({ hotel }: HotelCardProps) {
  ...
  const renderStars = (rating: number) => {
    ...
    return (
      <span className="text-gold text-micro tracking-wider leading-none">
  ...
  return (
    ...
    >
      <div className="aspect-[3/2] w-full shrink-0 relative overflow-hidden bg-secondary-surface group">
        {imgFailed ? (
      ...
      <div className="flex-1 p-5 flex flex-col justify-between gap-4">
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2 flex-wrap text-micro text-muted">
            ...
            <span className="text-border/60">|</span>
            <div className="flex items-center gap-0.5 text-teal font-mono uppercase tracking-wider text-micro font-bold">
          ...
          <div className="flex flex-wrap gap-1.5 pt-1">
            {hotel.amenities.slice(0, 3).map((a) => (
              <span key={a} className="px-2.5 py-0.5 rounded-full bg-background text-micro font-mono uppercase tracking-wider text-muted/70 border border-border/40 leading-none">{a}</span>
            ))}
            {hotel.amenities.length > 3 && (
              <span className="px-2.5 py-0.5 rounded-full bg-background text-micro font-mono uppercase tracking-wider text-muted/50 border border-border/40 leading-none">
                +{hotel.amenities.length - 3}
              </span>
            )}
          </div>
        ...
        <div className="flex items-center gap-2 w-full pt-3.5 border-t border-border/40">
          {Object.entries(hotel.affiliateLinks).filter(([, url]) => url).map(([key, url]) => (
            <motion.a key={key} href={url} target="_blank" rel="noopener noreferrer"
              ...
              className={`flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-micro font-bold uppercase tracking-wider border min-h-[38px] transition-all ${AFFILIATE_COLORS[key] || 'bg-background text-muted border-border/40'}`}
